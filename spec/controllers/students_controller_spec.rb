...
RSpec.describe StudentsController, type: :controller do
  ...
  describe 'when the authorization token did not arrive' do
    ...
    it 'returns forbidden 403 status' do
      expect { get :index, params: { school_id: school.id, class_id: school_class.id } }.to raise_error('Forbidden')
    end
  end

  describe 'GET index' do
    it 'has a 200 status code' do
      get :index, params: { school_id: school.id, class_id: school_class.id }

      expect(response.status).to eq(200)
    end

    it 'a response hash contain necessary keys' do
      get :index, params: { school_id: school.id, class_id: school_class.id }
      expected_records = [{ 'id' => student.id,
  ...
  describe 'GET create' do
    ...
    it 'increments students_count in class' do
      expect do
        post :create, params: {
          "student": {
            "first_name": 'Имя',
      ...
      end.to change { school_class.reload.students_count }.by(1)
    end
  end
end
